...
export async function criarCliente(req, res) {
  ...
  if (!nome || !segmento || !atividade) {
    return res
      .status(400)
      .json({ mensagem: "Os campos obrigatórios devem ser preenchidos." });
  }

  try {
    if (typeof Cliente.create !== "function") {
      console.error(
        "Erro: Método Cliente.create não encontrado ou inválido no modelo."
      );
      return res
        .status(500)
        .json({ mensagem: "Erro interno no servidor (modelo)." });
    }
...
export async function listarCidades(req, res) {
  ...
  try {
    const query =
      "SELECT DISTINCT Cidade FROM Cliente WHERE Cidade IS NOT NULL AND Cidade <> '' ORDER BY Cidade";
...
export async function listarSegmentos(req, res) {
  ...
  try {
    const query =
      "SELECT DISTINCT Segmento FROM Cliente WHERE Segmento IS NOT NULL AND Segmento <> '' ORDER BY Segmento";
...
export async function listarClientesParaDropdown(req, res) {
  ...
  if (!idColaborador) {
    return res
      .status(400)
      .json({ mensagem: "ID do colaborador é obrigatório." });
  }

  try {
    const [[colaborador]] = await db
      .promise()
      .query("SELECT Setor FROM Colaboradores WHERE ID_colaborador = ?", [
        idColaborador,
      ]);
