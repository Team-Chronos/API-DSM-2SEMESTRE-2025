  ...
  try {
    ...
    const checklistsConvertidos = checklists.map((checklist) => {
      ...
      for (const [key, value] of Object.entries(checklist)) {
        const snakeKey = key
          .toLowerCase()
          .replace(/_([a-z])/g, (match, p1) => p1.toLowerCase());
...
export const obterChecklistPorId = async (req, res) => {
  ...
  try {
    const [rows] = await db
      .promise()
      .query("SELECT * FROM ChecklistManutencao WHERE ID_Checklist = ?", [id]);
...
export const excluirChecklist = async (req, res) => {
  ...
  try {
    const [result] = await db
      .promise()
      .query("DELETE FROM ChecklistManutencao WHERE ID_Checklist = ?", [id]);
