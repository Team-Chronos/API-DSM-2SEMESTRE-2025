...
export const criarEvento = async (req, res) => {
  ...
  if (!nome_evento || !data_evento) {
    return res
      .status(400)
      .json({ mensagem: "O nome e a data do evento são obrigatórios!" });
  }
...
export const getParticipantesDoEvento = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    const [participantes] = await db.promise().query(query, [id]);
...
export const atualizarEvento = async (req, res) => {
  ...
  try {
    const query =
      "UPDATE Evento SET Nome_Evento = ?, Data_Evento = ?, Duracao_Evento = ?, local_evento = ?, ID_Tipo_Evento = ?, descricao = ? WHERE ID_Evento = ?";
    const [result] = await db
      .promise()
      .query(query, [
        nome_evento,
