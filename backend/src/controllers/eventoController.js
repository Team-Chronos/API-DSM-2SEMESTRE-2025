...
export const criarEvento = async (req, res) => {
    const {nome_evento, data_evento, duracao_evento, local_evento, tipo_evento, descricao_evento, participantes} = req.body;
    ...
    try {
        const query = 'INSERT INTO Evento (Nome_Evento, Data_Evento, Duracao_Evento, Local_Evento, ID_Tipo_Evento, Descricao) VALUES (?, ?, ?, ?, ?, ?)';
        const [result] = await db.promise().query(query, [nome_evento, data_evento, duracao_evento, local_evento, tipo_evento, descricao_evento]);
...
export const atualizarEvento = async (req, res) => {
    ...
    try {
        const query = "UPDATE Evento SET Nome_Evento = ?, Data_Evento = ?, Duracao_Evento = ?, local_evento = ?, ID_Tipo_Evento = ?, descricao = ? WHERE ID_Evento = ?";
        const [result] = await db.promise().query(query, [nome_evento, data_evento, duracao_evento, local_evento, tipo_evento, descricao_evento, id]);
