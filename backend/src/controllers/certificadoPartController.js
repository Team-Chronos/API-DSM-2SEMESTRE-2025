...
export const criarCertPartEvento = async (req, res) => {
    const { id_colaborador, id_evento, objetivo, principais_infos, aplicacoes_newe, referencias, avaliacao, comentarios} = req.body;
    
    try {
        const query = 'INSERT INTO Certificado_Participacao VALUES (?, ?, ?, ?, ?, ?, ?, ?, default)';
        const [result] = await db.promise().query(query, [id_colaborador, id_evento, objetivo, principais_infos, aplicacoes_newe, referencias, avaliacao, comentarios]);
