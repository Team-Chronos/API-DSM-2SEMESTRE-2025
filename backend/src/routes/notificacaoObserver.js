...
class NotificacaoObserver {
    ...
    async buscarIdsAtuais() {
        try {
            ...
            console.log(` IDs atuais - Eventos: ${this.ultimoEventoId}, Colaboradores: ${this.ultimoColaboradorId}, Agregados: ${this.ultimoAgregadoId}, Notificações: ${this.ultimoNotificacaoPersonalizadaId}`);
        } catch (error) {
    ...
    async verificarNotificacoesPersonalizadas() {
        try {
            ...
            for (const notificacao of notificacoes) {
                console.log(` Notificação personalizada detectada: ${notificacao.titulo}`);
        ...
        try {
            const notificacoesExemplo = [
                {
                    ...
                    mensagem: 'Agora você pode enviar notificações personalizadas para colaboradores através do sistema.',
                ...
                },
                {
                    ...
                    mensagem: 'Faremos uma manutenção no sistema no próximo sábado das 08h às 12h. O sistema ficará indisponível durante este período.',
                    destinatarios: JSON.stringify('todos'),
                    prioridade: 'alta',
                    criado_por: 1
                }
            ...
            for (const notificacao of notificacoesExemplo) {
                await db.promise().query(
                    ...
                    [notificacao.titulo, notificacao.mensagem, notificacao.destinatarios, notificacao.prioridade, notificacao.criado_por]
                );
            }
        ...
        try {
            ...
            if (notificacao.destinatarios === 'todos' || notificacao.destinatarios === '"todos"') {
                ...
                try {
                    ...
                    if (typeof notificacao.destinatarios === 'string' && !isNaN(notificacao.destinatarios)) {
            ...
            if (erros === 0) {
                ...
                console.log(` Notificação "${notificacao.titulo}" enviada para ${enviados} destinatários`);
            } else if (enviados > 0) {
                await this.marcarNotificacaoComoParcial(notificacao.id, enviados, erros);
                console.log(` Notificação "${notificacao.titulo}" enviada para ${enviados} destinatários, ${erros} erros`);
            } else {
                await this.marcarNotificacaoComoErro(notificacao.id, 'Falha ao enviar para todos os destinatários');
                console.log(` Notificação "${notificacao.titulo}" falhou para todos os destinatários`);
            }
    ...
    static async criarNotificacao(dadosNotificacao) {
        try {
            const { titulo, mensagem, destinatarios, prioridade = 'media', criado_por, agendamento = null } = dadosNotificacao;
            ...
            console.log(` Notificação personalizada criada: ${titulo} (ID: ${result.insertId})`);
