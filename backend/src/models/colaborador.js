...
const Colaborador = {
  findAllCargos: () => {
    const query =
      "SELECT ID_Cargo, Nome_Cargo FROM Cargo ORDER BY Nome_Cargo ASC";
  ...
  create: (data) => {
    ...
    const query =
      "INSERT INTO Colaboradores (Nome_Col, Email, Senha, Telefone, CPF, Setor, ID_Cargo) VALUES (?, ?, ?, ?, ?, ?, ?)";
    return db
      .promise()
      .query(query, [nome, email, senhaHash, telefone, cpf, setor, ID_Cargo]);
  },

  findAll: () => {
    const query =
      "SELECT ID_colaborador, Nome_Col, Telefone, Email, Setor, ID_Cargo, Localidade FROM Colaboradores";
    return db.promise().query(query);
  },

  findById: (id) => {
    const query =
      "SELECT ID_colaborador, Nome_Col, Telefone, Email, CPF, Setor, ID_Cargo FROM Colaboradores WHERE ID_colaborador = ?";
  ...
  updateLocalidade: (colaboradorId, localidade) => {
    const query =
      "UPDATE Colaboradores SET Localidade = ? WHERE ID_colaborador = ?";
