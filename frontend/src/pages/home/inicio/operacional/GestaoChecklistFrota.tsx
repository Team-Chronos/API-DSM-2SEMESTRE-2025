...
export function GestaoChecklistFrota() {
    ...
    const fetchChecklists = async () => {
        try {
            setLoading(true);
            const response = await api.get("/checklistVeiculoFrota");
    ...
    const calcularEstatisticas = (dados: any[]) => {
        ...
        dados.forEach(c => {
            const itensParaVerificar = [
                ...
                c.sistema_eletrico,
                c.estado_pneus,
                c.limpeza_bau_sider_cabine,
                c.lubrificacao_suspensoes,
                c.macaco,
                c.chave_roda,
                c.documento_vigente
            ];

            itensParaVerificar.forEach(item => {
                if (normalizarTexto(item) === 'nao') problemas++;
            });



            if (normalizarTexto(c.abastecimento) === 'sim' && normalizarTexto(c.comprovante_enviado) === 'nao') {
                problemas++;
            }
        });


        const totalItensAvaliados = Math.max(1, dados.length * 9);
    ...
    const temProblemas = (c: any): boolean => {
        ...
        const temFalhaCritica = camposCriticos.some(val => normalizarTexto(val) === 'nao');


        const pendenciaComprovante = normalizarTexto(c.abastecimento) === 'sim' && normalizarTexto(c.comprovante_enviado) === 'nao';

        return temFalhaCritica || pendenciaComprovante;
    };
        ...
        <div className="container mt-4">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div className="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h1 className="h3 mb-1">Checklist Veículo Frota</h1>
                        <p className="text-muted mb-0">Gestão e monitoramento da frota própria</p>
                    </div>
                </div>
                <Button
                    variant="primary"
                            ...
                            <table className="table table-hover align-middle">
                                ...
                                <tbody>
                                    {checklists.map((item) => {
                                        ...
                                        return (
                                            <tr key={item.id_cvf} className={problematico ? 'table-warning' : ''}>
                                                <td><Badge bg="secondary">#{item.id_cvf}</Badge></td>
                                                <td>{formatarDataHora(item.criado_em)}</td>
                                                <td>
                                                    <div className="fw-bold">{item.nome_motorista || "—"}</div>
                                                    {item.id_motorista !== 'outro' && (
                                                        <small className="text-muted">ID: {item.id_motorista}</small>
                                                    )}
                                                </td>
                                                <td>
